...
// yaw is initialized to -90.0 degrees since a yaw of 0.0 results in a direction vector pointing to the right so we initially rotate a bit
float yaw        = -90.0f;
// to the left.
float pitch      = 0.0f;
...
int main() {
    ...
    SDL_Window   *window  = SDL_CreateWindow("7.3.camera_mouse_zoom", window_width, window_height, SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // load image, create texture and generate mipmaps
    data = stbi_load("../../resources/textures/images.png", &width, &height, &nrChannels, 0);
    if (data) {
    ...
    while (run) {
        ...
        float cameraSpeed = static_cast<float>(cameraSpeedFactor * deltaTime);
        // input
        // -----
