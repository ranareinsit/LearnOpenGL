...
int main() {
    ...
    SDL_Window   *window  = SDL_CreateWindow("6.3.coordinate_systems_multiple", window_width, window_height, SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    ...
    while (run) {
        ...
        ourShader.setMat4("projection", projection); // note: currently we set the projection matrix each frame, but since the projection matrix rarely changes
