...
int main() {
    ...
    SDL_Window *window =
        SDL_CreateWindow("4-advanced_opengl/5-2-framebuffers_exercise1", SCR_WIDTH, SCR_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    ...
    float quadVertices[] = {
        ...
        // vertex attributes for a quad that fills the entire screen in Normalized Device Coordinates. NOTE that this plane is now much
        // smaller and at the top of the screen
        // positions   // texCoords
        -0.3f, 1.0f, 0.0f, 1.0f, -0.3f, 0.7f, 0.0f,
        0.0f,  0.3f, 0.7f, 1.0f, 0.0f,

        -0.3f, 1.0f, 0.0f, 1.0f, 0.3f,  0.7f, 1.0f,
    ...
    while (run) {
        ...
        camera.ProcessMouseMovement(0, 0, false); // call this to make sure it updates its camera vectors, note that we disable pitch constrains for this
                                                  // specific case (otherwise we can't reverse camera's pitch values)
