...
int main() {
    ...
    SDL_Window   *window  = SDL_CreateWindow("4-advanced_opengl/2-stencil_testing", SCR_WIDTH, SCR_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    ...
    while (run) {
        ...
        // draw floor as normal, but don't write the floor to the stencil buffer, we only care about the containers. We set its mask to 0x00 to not write to the
        // stencil buffer.
        glStencilMask(0x00);
        // floor
        glBindVertexArray(planeVAO);
