...
class ISoundEngine {
  public:
    ...
    std::map<std::string, Sound> queue;
    ...
    bool init_sound(const char *fname) {
        bool          retval = false;
        SDL_AudioSpec spec;

        if (sounds.contains(fname)) {
            return false; // Sound already loaded
        }

        Sound *temp  = &sounds[fname];
        temp->active = false;

        // Load the .wav file
        if (!SDL_LoadWAV(fname, &spec, &temp->wav_data, &temp->wav_data_len)) {
            SDL_Log("Couldn't load .wav file: %s", SDL_GetError());
            return false;
        }

        // Create an audio stream
        temp->stream = SDL_CreateAudioStream(&spec, NULL);
        if (!temp->stream) {
            SDL_Log("Couldn't create audio stream: %s", SDL_GetError());
        } else if (!SDL_BindAudioStream(audio_device, temp->stream)) {
    ...
    void play(const std::string &path) {
        ...
        if (it != sounds.end()) {
            ...
            // Reset the stream before playing

            // Put the audio data into the stream
            //  SDL_PutAudioStreamData(sound.stream, sound.wav_data, (int)sound.wav_data_len);

            // Play the sound by flushing the stream (this assumes your binding is correct)
            // Note: You might need to call a function to start playback depending on your setup.
            // For example: SDL_QueueAudio(audio_device, sound.wav_data, sound.wav_data_len);
        } else {
            SDL_Log("Sound not found: %s", path.c_str());
        }
    }

    void track_progress(Sound &sound) {
        // Get the amount of audio data currently queued for playback
        int queued = SDL_GetAudioStreamQueued(sound.stream);

        // Get the amount of audio data available for playback
        int available = SDL_GetAudioStreamAvailable(sound.stream);

        // Calculate the total size of the buffer (total length of sound data)
        int total_size = sound.wav_data_len;

        // Calculate how much has been played
        int played = total_size - queued; // Amount consumed

        // Output the current status
        std::cout << "Total: " << total_size << " Current consumed: " << played << " available: " << available << " queued: " << queued << " bytes"
                  << std::endl;
    }

    void drain() {
        for (auto &sound : sounds) {
            if (sound.second.active == false) {
                continue;
            }

            // TODO: sound queue
            // track_progress(sound.second);
            if (sound.second.wav_data_len - SDL_GetAudioStreamQueued(sound.second.stream) >= (sound.second.wav_data_len - 100)) {
                // queue.erase(sound.first);
                sound.second.active = false;
            } else if (SDL_GetAudioStreamAvailable(sound.second.stream) < ((int)sound.second.wav_data_len)) {
                SDL_PutAudioStreamData(sound.second.stream, sound.second.wav_data, (int)sound.second.wav_data_len);
            }
        }
    }
    ...
    ~ISoundEngine() {
        // Clean up resources
        for (auto &sound : sounds) {
            SDL_free(sound.second.wav_data);             // Free WAV data
            SDL_DestroyAudioStream(sound.second.stream); // Free audio stream
        }
        ...
        if (audio_device != 0) {
            SDL_CloseAudioDevice(audio_device);          // Close audio device
        }
    }
};
...
Game::Game(unsigned int width, unsigned int height) : State(GAME_MENU), Keys(), KeysProcessed(), Width(width), Height(height), Level(0), Lives(3) {}
...
void Game::Init() {
    ...
    // audio
    SoundEngine->init_sound("../../../resources/audio/breakout.wav");
    SoundEngine->init_sound("../../../resources/audio/powerup.wav");
    SoundEngine->init_sound("../../../resources/audio/bleep.wav");
    SoundEngine->play("../../../resources/audio/breakout.wav");
}
...
void Game::UpdatePowerUps(float dt) {
    ...
    this->PowerUps.erase(
        std::remove_if(this->PowerUps.begin(), this->PowerUps.end(), [](const PowerUp &powerUp) { return powerUp.Destroyed && !powerUp.Activated; }),
        this->PowerUps.end());
}
...
void Game::DoCollisions() {
        ...
        if (!box.Destroyed) {
            ...
            if (std::get<0>(collision)) // if collision is true
            {
                ...
                if (!box.IsSolid) {
                    ...
                    SoundEngine->play("../../../resources/audio/bleep.wav");
    ...
    for (PowerUp &powerUp : this->PowerUps) {
        if (!powerUp.Destroyed) {
            ...
            if (CheckCollision(*Player, powerUp)) { // collided with player, now activate powerup
                ...
                SoundEngine->play("../../../resources/audio/powerup.wav");
            }
        }
    }
    ...
    if (!Ball->Stuck && std::get<0>(result)) {
        ...
        Ball->Velocity =
            ...
            glm::length(oldVelocity); // keep speed consistent over both axes (multiply by length of old velocity, so total strength is not changed)
        // fix sticky paddle
...
Collision CheckCollision(BallObject &one, GameObject &two) // AABB - Circle collision
{
    ...
    if (glm::length(difference) < one.Radius) // not <= since in that case a collision also occurs when object one exactly touches object two, which they are at
