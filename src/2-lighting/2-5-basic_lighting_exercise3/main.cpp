...
int main() {
    ...
    SDL_Window   *window  = SDL_CreateWindow("2-5-basic_lighting_exercise3", SCR_WIDTH, SCR_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    ...
    while (run) {
        ...
        // lightingShader.setFloat("iTimeDelta", sin(delta / 2.0f));

        // change the light's position values over time (can be done anywhere in the render loop actually, but try to do it at least before using the light
        // source positions)
        lightPos.x = 1.0f + sin(delta) * 2.0f;
        lightPos.y = sin(delta / 2.0f) * 1.0f;
