...
// --------------------
void renderScene(const Shader &shader, float delta) {
    // room cube
    ...
    model = glm::rotate(model, glm::radians(delta * 10), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
    model = glm::scale(model, glm::vec3(0.75f));
    shader.setMat4("model", model);
    renderCube();
}
...
int main() {
    ...
    SDL_Window *window =
        SDL_CreateWindow("5-advanced_lighting/3-2-2-point_shadows_soft", SCR_WIDTH, SCR_HEIGHT, SDL_WINDOW_OPENGL | SDL_WINDOW_HIGH_PIXEL_DENSITY);
    ...
    while (run) {
        ...
        renderScene(simpleDepthShader, delta);
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
